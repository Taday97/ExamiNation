...
namespace ExamiNation.Application.Validators.Option
{
    public class EditOptionDtoValidator : AbstractValidator<EditOptionDto>
    {
        ...
        public EditOptionDtoValidator()
        {
            RuleFor(x => x.Id)
                .NotEmpty()
                .Must(Id => Guid.TryParse(Id.ToString(), out _)).WithMessage("Option ID must be a valid GUID.")
                .Must((dto, Id) => Id == dto.Id).When(dto => !string.IsNullOrEmpty(dto.Id.ToString())).WithMessage("Option ID in request body does not match the ID in the URL.");
